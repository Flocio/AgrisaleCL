...
class _SalesScreenState extends State<SalesScreen> {
  ...
  void _filterSales() {
    ...
    setState(() {
      List<Sale> result = List.from(_sales);
      ...
      if (searchTerms.isNotEmpty) {
        ...
        result = result.where((sale) {
          ...
          final price = (sale.totalSalePrice ?? 0).toString().toLowerCase();
      ...
      if (_selectedCustomerFilter != null) {
        ...
        result = result.where((sale) => 
          sale.customerId == selectedCustomer.id).toList();
      }
    ...
    if (result != null) {
      try {
        final saleCreate = SaleCreate(
          ...
          saleDate: result['saleDate'] as String?,
          customerId: result['customerId'] as int?,
          totalSalePrice: result['totalSalePrice'] as double?,
          note: result['note'] as String?,
        );
        
        await _saleRepo.createSale(saleCreate);
        _fetchData();
        ...
        if (mounted) {
          _showErrorDialog('添加销售记录失败: ${e.toString()}');
        }
      }
    }
  }
  ...
  Future<void> _editSale(Sale sale) async {
    ...
    if (result != null) {
      try {
        ...
        if (mounted) {
          _showErrorDialog('更新销售记录失败: ${e.toString()}');
        }
      }
    }
  }

  void _showNoteDialog(Sale sale) {
  ...
  Future<void> _deleteSale(Sale sale) async {
    ...
    if (confirm == true) {
      try {
        ...
        if (mounted) {
          _showErrorDialog('删除销售记录失败: ${e.toString()}');
        }
      }
    }
  }
...
class SalesDialog extends StatefulWidget {
  ...
  final Sale? existingSale; // 添加此参数用于编辑模式
    ...
    final product = widget.products.firstWhere((p) => p.name == _selectedProduct);
      setState(() {
        ...
        if (_isEditMode && widget.existingSale != null) {
          final oldProductName = widget.existingSale!.productName;
          if (oldProductName == _selectedProduct) {
            // 如果编辑时没有改变产品，加上原数量
            final oldQuantity = widget.existingSale!.quantity;
            ...
            DropdownButtonFormField<String>(
              ...
              items: widget.products.map((product) {
                return DropdownMenuItem<String>(
                  value: product.name,
                    child: Text('${product.name} (库存: ${_formatNumber(product.stock)} ${product.unit.value})'),
                );
              }).toList(),
                validator: (value) {
