...
**默认值：**
- `DB_PATH="data/agrisalecl.db"` - 数据库文件路径
...
- **FastAPI 服务器**（`./start.sh`）：提供实际的 API 服务，监听 `localhost:8000`
...
```bash
# 创建隧道（名称可自定义）
cloudflared tunnel create agrisalecl-server
```
...
cloudflared tunnel route dns agrisalecl-server agrisalecl.drflo.org
```
...
sudo cloudflared tunnel run agrisalecl-server
```
...
curl https://agrisalecl.drflo.org/health
```
...
完成快速开始和内网穿透配置后，可以将两个服务都配置为 systemd 服务，实现开机自启和自动重启。
...
Cloudflare Tunnel 在"内网穿透配置"章节的第 7 步已经配置为系统服务，无需重复配置。
...
修改 `/home/florin/Projects/AgrisaleCL/server/` 下的代码后，需要重启服务使更改生效：

```bash
# 重启 FastAPI 服务
sudo systemctl restart agrisalecl

# 查看状态确认
sudo systemctl status agrisalecl

# 查看日志确认更新成功
sudo journalctl -u agrisalecl -n 20 -f
```
...
2. **在 `ExecStart` 行添加 `--reload` 参数：**
   ```ini
   ExecStart=/path/to/server/venv/bin/python -m uvicorn server.main:app --host 0.0.0.0 --port 8000 --reload
   ```
